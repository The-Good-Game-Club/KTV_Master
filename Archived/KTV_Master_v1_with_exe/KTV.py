...
def remux(video_path: Path, accomp_path: Path, output_dir: Path,
          subtitle_path: Path | None = None) -> Path:
    stem = video_path.stem
    if subtitle_path:
        out_name = stem + "_instrumental_karaoke.mp4"
    else:
        out_name = stem + "_instrumental.mp4"
    ...
    while out_path.exists():
        suffix = f"_{counter}"
        if subtitle_path:
            out_name = stem + suffix + "_instrumental_karaoke.mp4"
        else:
            out_name = stem + suffix + "_instrumental.mp4"
        out_path = output_dir / out_name
        counter += 1
